...
try {
  ...
  log(`Failed to load .env file: ${error instanceof Error ? error.message : 'Unknown error'}`, 'error');
  process.exit(1);
}
...
try {
  ...
  log(`Failed to initialize wallet: ${error instanceof Error ? error.message : 'Unknown error'}`, 'error');
  process.exit(1);
}

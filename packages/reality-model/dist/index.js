...
dotenv.config();
const PRIVATE_API_KEY = process.env.PRIVATE_ANTHROPIC_API_KEY;
const app = express();
const port = 3030;
// Enable CORS
app.use(cors());
...
function constructMessages(roomPrompt, ratPrompt) {
    const messages = [];
...
app.post('/api/generate', async (req, res) => {
    try {
        const { realityPrompt, styleGuidelines, roomPrompt, ratPrompt } = req.body;
        const systemPrompt = `${realityPrompt} ${styleGuidelines}`;
        const messages = constructMessages(roomPrompt, ratPrompt);

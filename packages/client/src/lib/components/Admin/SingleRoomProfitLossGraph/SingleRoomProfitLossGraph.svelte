<script lang="ts">
  ...
  let plotData = $derived.by(() => {
    ...
    const initialPoint = {
      ...
      roomValueChange: -Number(trip.roomCreationCost), // Initial investment is negative
  ...
  // Accumulate value changes for the single room
  let allData = $derived.by(() => {
    ...
    return plotData.map((point, index) => {
      ...
      return {
        ...
        index,
        tripId,
        roomCreationCost: trip.roomCreationCost,

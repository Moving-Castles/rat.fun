...
const [mainnet, testnet] = await Promise.all([
  ...
  fetch("https://api.testnets.relay.link/chains").then(
    res => res.json() as Promise<RelayChainsApiResponse>
  )
])
...
const chains = Object.fromEntries<RelayChain>([
  ...testnet.chains.map(
    ({ id, name }) => [id, { bridgeUrl: `https://testnets.relay.link/bridge/${name}` }] as const
  ),
  ...mainnet.chains.map(
    ({ id, name }) => [id, { bridgeUrl: `https://relay.link/bridge/${name}` }] as const
  )

...
if [ -z "$CHAIN_ID" ]; then
    ...
    echo "    8453  - base-mainnet"
    echo "    84532 - base-sepolia"
    echo "  Value: 0 or higher (number of rats)"
    exit 1
fi
...
if ! [[ "$VALUE" =~ ^[0-9]+$ ]] || [ "$VALUE" -lt 0 ]; then
    echo "Error: Value must be a non-negative number"
    exit 1
fi
...
echo "Setting RatsKilledForAdminAccess to: $VALUE"
echo ""

forge script SetRatsKilledForAdminAccess --sig run\(address,uint32\) $WORLD_ADDRESS $VALUE --broadcast --rpc-url $RPC -vvvv

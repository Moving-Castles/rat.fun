...
async function confirmDeletion(): Promise<boolean> {
  ...
  return new Promise(resolve => {
    readline.question(
      '\nWARNING: This will delete A LOT of documents from Sanity. Type "I have become death" to confirm: ',
      (answer: string) => {
        readline.close()
...
async function purgeWorld() {
  try {
    console.log(
      `Starting purge${deleteAll ? " for ALL documents" : ` for world address: ${worldAddress}`}`
    )
